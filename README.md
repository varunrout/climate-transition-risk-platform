...
| M6 | Energy-system transition features | **Phases 1-3 implemented and promoted to local production; Azure promotion pending.** Phase 1 (`docs/m6_source_feasibility.md`, ADR 0007): OWID `energy-data` verified and ingested. Phase 2 (ADR 0008) evaluated a 3-signal energy component and decided ACCEPT. Phase 3 (`climate-risk m6-harden`, ADR 0009) strengthened the evidence (2000-permutation test), tested redundancy-reduced alternatives, and froze a leaner **2-signal** production spec (`energy_component_v2.1`: power-system dependence + transition momentum) after finding the original 3rd signal near-mechanically redundant. `cli.score()` now computes **both v1 and v2**; `cli.publish()` **requires both** and declares v2 (`score_version=v2_energy`) as the active production score while permanently preserving v1 as a comparison artifact (`gold/country_transition_risk.parquet` unchanged; v2 in `gold/country_transition_risk_v2.parquet`). Verified locally end-to-end (163 tests passing, fail-closed tested). **The deployed Azure job has not yet run this — Azure promotion (image rebuild/push, Terraform image update, one manual execution, local/Azure parity check) is the remaining step before M6 is COMPLETE.** See ADR 0009 for full evidence, caveats (lookback-window sensitivity of the retained momentum signal; one historical backtest origin where energy features degrade performance), and the frozen specification. |
