...
| M7 | Regime/structural-break research | **Phases 1-3 implemented** (research-only). `climate-risk m7-phase1` writes leakage-safe structural-break diagnostics; `m7-phase2` recomputes regimes at historical origins; `m7-phase3` compares regime-aware scenarios against production baselines and a recency-only control. Phase 3 decision: **RECENCY_WEIGHTING_ONLY**; regime-aware production integration is not justified. No production score, scenario engine, Azure schedule, or publish contract change. See ADR 0011-0013 and `docs/m7_phase3_report.md`. |
