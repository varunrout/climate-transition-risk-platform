...
@pytest.fixture
def lake_with_bronze(tmp_path: Path) -> RunPaths:
    paths = RunPaths(lake_root=tmp_path / "lake")
    paths.ensure_zones()

    owid_dir = paths.bronze / "source=owid_co2" / "snapshot_id=abc123"
    owid_dir.mkdir(parents=True)
    pd.DataFrame(
        {
    ...
    ).to_parquet(owid_dir / "data.parquet", index=False)

    wb_dir = paths.bronze / "source=world_bank_wdi" / "snapshot_id=def456"
    ...
    ).to_parquet(wb_dir / "data.parquet", index=False)

    return paths


def test_build_silver_panel_joins_and_computes_intensity(lake_with_bronze: RunPaths) -> None:
...
def test_no_duplicate_keys_and_no_aggregate_leakage(lake_with_bronze: RunPaths) -> None:
...
def test_latest_complete_common_year_requires_all_countries(lake_with_bronze: RunPaths) -> None:
...
def test_write_silver_zone_is_readable_back(lake_with_bronze: RunPaths) -> None:
    panel, snapshot_set_id, _ = build_silver_panel(lake_with_bronze)
    write_dim_country(build_dim_country(), paths=lake_with_bronze)
    write_fact_country_year_transition(
        panel, snapshot_set_id=snapshot_set_id, paths=lake_with_bronze
    )

    dim = pd.read_parquet(lake_with_bronze.silver / "dim_country" / "data.parquet")
    assert len(dim) == 19  # full G20 sovereign panel, independent of which countries have data

    fact_path = (
        lake_with_bronze.silver
        / "fact_country_year_transition"
        / f"snapshot_set_id={snapshot_set_id}"
        / "data.parquet"
    )
    assert fact_path.exists()
    reread = pd.read_parquet(fact_path)
    assert len(reread) == len(panel)


def test_missing_bronze_snapshot_raises_clear_error(tmp_path: Path) -> None:
    paths = RunPaths(lake_root=tmp_path / "empty_lake")
    paths.ensure_zones()
    with pytest.raises(FileNotFoundError, match="no accepted bronze snapshot"):
        build_silver_panel(paths)
